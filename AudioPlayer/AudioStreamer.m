...
- (void)presentAlertWithTitle:(NSString*)title message:(NSString*)message
{
	...
	NSAlert *alert =
		[NSAlert
			...
			defaultButton:NSLocalizedString(@"确定", @"")
			alternateButton:nil
			otherButton:nil
...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		[self presentAlertWithTitle:NSLocalizedStringFromTable(@"警告", @"Errors", nil)
							message:NSLocalizedStringFromTable(@"网络连接异常", @"Errors", nil)];
	}
}
	...
	eventType:(CFStreamEventType)eventType
{
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		if (!audioFileStream)
		{
			...
			{
				self.fileExtension = [[_url path] pathExtension];
			}
